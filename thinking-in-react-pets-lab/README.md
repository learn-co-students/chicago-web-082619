...
2) Build Static version in React
    - nothing user-interactive yet
    - props only, no state
    - still using real data model and React UI components
3) Identify minimal (but complete) representation of UI state
    - DRY DRY DRY! (Don't need state.arrayCount if you have state.array)
    - State is what changes over time AND can't be computed from anything else
4) Identify where your state should live
    - Often the hardest thing to get the hang of!
    - For each piece of state in the app:
      * What components need to render something based on this?
      * Is there a sensible common owner of all the components? Give it the state!
      * Anything higher up in the common owner's "ancestry" will work too!
      * If there's no sensible common owner (maybe the component's job doesn't really match the state it would be holding), you can make a new one higher up
5) Add inverse data flow
    - Props are already being passed "down"
    - Now we start to pass functions down with them
    - When the child runs a function, data flows "up" to change parent state.
6) That's it!
